...
export async function addIlluminationDataToCelestialDays(year: number, days: ICelestialDay[], timeShift: number): Promise<void> {
    ...
    for (const day of days) {
        ...
        if (illumination) {
            day.illuminationPercentage = illumination.moonProps.illuminationPercentage;
        } else {
...
function __fillWaxingOrWaning(illuminationData: IIlluminationDate[]): void {
    ...
    for (let i = 0; i < illuminationData.length; i++) {
        const thisDay = illuminationData[i].moonProps.illuminationPercentage;
        const nextDay = (() => {
            let ii = i + 1;
            while (ii < illuminationData.length && illuminationData[ii].moonProps.illuminationPercentage === thisDay) { ii++; }
            if (ii >= illuminationData.length) return undefined;
            return illuminationData[ii].moonProps.illuminationPercentage;
        })();
        if (nextDay == undefined) {
            illuminationData[i].moonProps.waxingOrWaning = lastWaxingWaning;
        } else {
